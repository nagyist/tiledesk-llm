...
async def _index_tables_to_vector_store(
    ...
    for table_data in tables:
        ...
        meta = CommonChunkMetadata(
            ...
            type='table',
            element_type='table', # Map to situated context element type
            ...
            col_names=", ".join(table_data.get('columns', [])), # Map to situated context col_names
            surrounding_text=table_data.get('surrounding_text', ''),
    ...
    sc_config = getattr(question, 'situated_context', None)
    if sc_config and sc_config.enable:
        # NOTE: Atomic table descriptions generated by TableSemanticLinker are already context-rich
        # and include surrounding text references. Adding situated context here often results 
        # in redundant "double description" noise.
        # We only apply situated context if the description is NOT already a semantic one.
        docs_to_enrich = []
        for i, doc in enumerate(documents):
             # Check if the original table_data had a semantic description
             if not tables[i].get('semantic_description'):
                 docs_to_enrich.append(doc)
             else:
                 # It's already semantic, so we consider it "situated" by design
        ...
        if docs_to_enrich:
            try:
                ...
                if sc_llm:
                    ...
                    sc_token_usage = sc_result.token_usage
                    logger.info(f"Situated context applied to {len(docs_to_enrich)} basic table descriptions. Tokens: {sc_token_usage}")
            except Exception as sc_err:
                logger.warning(f"Situated context enrichment for tables failed, continuing without: {sc_err}")
        else:
            logger.info(f"Skipped situated context LLM call for {len(documents)} semantic table descriptions (flag set to True)")
...
async def _index_images_to_vector_store(
    ...
    for image_data in images:
        ...
        meta = CommonChunkMetadata(
            ...
            type='image',
            element_type='image', # Map to situated context element type
    ...
    sc_config = getattr(question, 'situated_context', None)
    if sc_config and sc_config.enable:
        # Similar to tables, image captions are already context-rich.
        # We only apply situated context if the caption is NOT a rich one.
        docs_to_enrich = []
        for i, doc in enumerate(documents):
             # Logic for identifying rich captions vs generic ones
             caption_text = images[i].get('caption', '')
             if "Image extracted from page" in caption_text:
                 # This is a generic placeholder, we NEED situated context
                 docs_to_enrich.append(doc)
             else:
                 # It's a rich caption, skip LLM call but set flag
                 doc.metadata["has_situated_context"] = True
        ...
        if docs_to_enrich:
            try:
                ...
                if sc_llm:
                    ...
                    sc_token_usage = sc_result.token_usage
                    logger.info(f"Situated context applied to {len(docs_to_enrich)} basic image captions. Tokens: {sc_token_usage}")
            except Exception as sc_err:
                logger.warning(f"Situated context enrichment for images failed, continuing without: {sc_err}")
        else:
            logger.info(f"Skipped situated context LLM call for {len(documents)} rich image captions (flag set to True)")
